...
function modelsForProvider(provider: Provider, chatGPTModels: ModelOption[], copilotModels: ModelOption[]) {
...
function reasoningModelFor(provider: Provider, modelId: string, models: ModelOption[]) {
  ...
  return provider === "chatgpt" ? models.find((model) => model.isDefault) || models[0] : undefined;
}
...
export default function SupervisorWorkspace() {
  ...
  const models = active ? modelsForProvider(active.provider, chatGPTModels, copilotModels) : [];
  ...
  useEffect(() => {
    ...
    setChats((current) => {
      ...
      const next = current.map((chat) => {
        ...
        const availableModels = modelsForProvider(chat.provider, chatGPTModels, copilotModels);
  ...
  }, [hydrated, chatGPTModels, copilotModels]);
  ...
  async function startSupervisorRun(chatId: string, explicitPrompt?: string, alreadyAppended = false, attachments: Attachment[] = []) {
    ...
    try {
      ...
      const runReasoningEffort = supportedReasoningEffort(thread.provider, thread.model, modelsForProvider(thread.provider, chatGPTModels, copilotModels), thread.reasoningEffort);
      const payload = thread.mode === "pr"
        ...
        : { mode: thread.mode, repo: thread.repo, branch: thread.branch, model: thread.model, reasoningEffort: runReasoningEffort, prompt: requestPrompt, attachments };
  ...
  async function startAgentRun(chatId: string, name: string) {
    ...
    try {
      const runReasoningEffort = supportedReasoningEffort(parent.provider, parent.model, modelsForProvider(parent.provider, chatGPTModels, copilotModels), parent.reasoningEffort);
