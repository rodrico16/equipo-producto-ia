...
function normalizeChat(input: Partial<ChatThread>): ChatThread {
  ...
  return { ...base, running: false, messages: supervisorMessages.slice(-200), agentThreads: threads, queuedSupervisor: input.queuedSupervisor || [] };
}
...
export default function SupervisorWorkspace() {
  ...
  function handleSupervisorEvent(chatId: string, runId: string, provider: Provider, model: string, event: StreamEvent) {
    ...
    if (event.type === "subagent.started" || event.type === "subagent.selected") {
      ...
      if (event.type === "subagent.started") system(chatId, `Supervisor delegó trabajo a ${displayName(name)}`);
      return;
    }
    ...
    if (event.type === "control.done") {
      ...
      updateChat(chatId, (chat) => ({ ...chat, running: false, status: "Turno completado", prUrl: prUrl || chat.prUrl, diffStat: diffStat || chat.diffStat, error: "", updatedAt: Date.now() })); return;
    }
  ...
  return (
    <main className={`${s.app} ${mobileListOpen ? s.showList : ""} ${mobileAgentsOpen ? s.showAgents : ""}`}>
      ...
      <section className={s.supervisorPane}>
        <header className={s.chatHeader}><button className={s.mobileBack} onClick={() => setMobileListOpen(true)}>‹</button><span className={s.supervisorAvatar}>S</span><div className={s.headerCopy}><strong>Supervisor</strong><span>{active.title} · {active.running ? active.status : "listo"}</span></div><button className={s.agentToggle} onClick={() => setMobileAgentsOpen(true)}>{parallelThreads.length ? `${parallelThreads.length} agentes` : "Agentes"}</button><button className={s.iconButton} onClick={() => setSettingsOpen(true)}>⚙</button></header>
