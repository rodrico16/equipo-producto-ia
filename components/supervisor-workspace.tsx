...
type Provider = "chatgpt" | "qwen" | "copilot" | "apple";
...
const QWEN_FALLBACK: ModelOption[] = [{ id: "qwen3-coder-plus", displayName: "Qwen3 Coder Plus" }];
...
function modelsForProvider(provider: Provider, chatGPTModels: ModelOption[], copilotModels: ModelOption[], qwenModels: ModelOption[] = QWEN_FALLBACK) {
  if (provider === "copilot") return copilotModels;
  if (provider === "qwen") return qwenModels.length ? qwenModels : QWEN_FALLBACK;
  if (provider === "apple") return [APPLE_INTELLIGENCE_MODEL];
  return chatGPTModels;
}
function reasoningModelFor(provider: Provider, modelId: string, models: ModelOption[]) {
  const selected = models.find((model) => model.id === modelId);
  if (selected) return selected;
  return provider === "chatgpt" || provider === "qwen" ? models.find((model) => model.isDefault) || models[0] : undefined;
}
function reasoningEffortsFor(model?: ModelOption) {
  return (model?.reasoningEfforts || []).filter((effort) => effort.id);
}
function supportedReasoningEffort(provider: Provider, modelId: string, models: ModelOption[], effort: string) {
  if (provider === "apple" || !effort) return "";
  const options = reasoningEffortsFor(reasoningModelFor(provider, modelId, models));
  return options.some((option) => option.id === effort) ? effort : "";
}
...
export default function SupervisorWorkspace() {
  ...
  const [qwen, setQwen] = useState<QwenState>({ status: "disconnected", models: QWEN_FALLBACK });
  ...
  useEffect(() => {
    ...
    fetch("/api/qwen/status", { cache: "no-store" }).then((r) => r.json()).then((body) => setQwen(body as QwenState)).catch(() => undefined);
  }, []);
  ...
  const qwenModels = qwen.models || QWEN_FALLBACK;
  const models = active ? modelsForProvider(active.provider, chatGPTModels, copilotModels, qwenModels) : [];
  const selectedModel = models.find((m) => m.id === active?.model);
  const reasoningModel = active ? reasoningModelFor(active.provider, active.model, models) : undefined;
  const reasoningEffortOptions = reasoningEffortsFor(reasoningModel);
  const activeReasoningEffort = active ? supportedReasoningEffort(active.provider, active.model, models, active.reasoningEffort) : "";
  const githubConnected = Boolean(session?.githubConnected);
  const chatGPTConnected = chatGPT.status === "connected";
  const qwenConnected = qwen.status === "connected";
  const supervisorAttachments = pendingAttachments[active?.id || ""] || [];
  const providerReady = active?.provider === "copilot" ? githubConnected : active?.provider === "qwen" ? qwenConnected : active?.provider === "apple" ? isIPhone : chatGPTConnected;
  ...
  const sendHint = active?.running && supervisorAttachments.length
    ...
    : active?.provider === "copilot" && supervisorAttachments.some((file) => file.type.startsWith("image/"))
      ? "Para analizar imágenes elegí ChatGPT / Codex o Qwen."
    ...
    : active.provider === "copilot" && !githubConnected
      ...
      : active.provider === "chatgpt" && !chatGPTConnected
        ...
        : active.provider === "qwen" && !qwenConnected
          ? "Configurá DASHSCOPE_API_KEY para usar Qwen."
  ...
  useEffect(() => {
    ...
    setChats((current) => {
      ...
      const next = current.map((chat) => {
        if (chat.provider === "chatgpt" && chat.reasoningEffort && !chatGPTModels.length) return chat;
        if (chat.provider === "copilot" && chat.reasoningEffort && copilotModels.length === 1 && copilotModels[0].id === "auto" && chat.model !== "auto") return chat;
        const availableModels = modelsForProvider(chat.provider, chatGPTModels, copilotModels, qwenModels);
        const reasoningEffort = supportedReasoningEffort(chat.provider, chat.model, availableModels, chat.reasoningEffort);
        if (reasoningEffort === chat.reasoningEffort) return chat;
        changed = true;
        return { ...chat, reasoningEffort };
      });
      return changed ? next : current;
    });
  }, [hydrated, chatGPTModels, copilotModels, qwenModels]);
  ...
  async function startSupervisorRun(chatId: string, explicitPrompt?: string, alreadyAppended = false, attachments: Attachment[] = []) {
    ...
    try {
      ...
      const runReasoningEffort = supportedReasoningEffort(thread.provider, thread.model, modelsForProvider(thread.provider, chatGPTModels, copilotModels, qwenModels), thread.reasoningEffort);
      const payload = thread.mode === "pr"
        ? { repo: thread.repo, branch: thread.branch, existingPrNumbers, provider: thread.provider, model: thread.model, reasoningEffort: runReasoningEffort, prompt: requestPrompt, attachments }
        : { mode: thread.mode, repo: thread.repo, branch: thread.branch, provider: thread.provider, model: thread.model, reasoningEffort: runReasoningEffort, prompt: requestPrompt, attachments };
  ...
  async function retryPublication(chatId: string) {
    ...
    if (!chat?.retryPrompt || chat.running || !githubConnected || (chat.provider === "chatgpt" && !chatGPTConnected) || (chat.provider === "qwen" && !qwenConnected) || retryLocks.current.has(chatId)) return;
  ...
  async function startAgentRun(chatId: string, name: string) {
    ...
    try {
      const runReasoningEffort = supportedReasoningEffort(parent.provider, parent.model, modelsForProvider(parent.provider, chatGPTModels, copilotModels, qwenModels), parent.reasoningEffort);
      const response = await fetch("/api/agent-run", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: parent.provider, agentName: name, prompt: requestPrompt, repo: parent.repo, branch: parent.branch, model: parent.model, reasoningEffort: runReasoningEffort, attachments }), signal: controller.signal });
      if (!response.ok || !response.body) throw new Error((await response.json().catch(() => ({}))).error || `HTTP ${response.status}`);
      const reader = response.body.getReader();
      activeRunsRef.current.get(runId)!.reader = reader;
      const decoder = new TextDecoder(); let buffer = "";
  ...
  return (
    <main className={`${s.app} ${mobileListOpen ? s.showList : ""} ${mobileAgentsOpen ? s.showAgents : ""}`}>
      ...
      <section className={s.supervisorPane}>
        ...
        <div className={s.contextBar}><span>{modeLabel(active.mode)}</span><span>{providerLabel(active.provider)} · {selectedModel?.displayName || selectedModel?.name || reasoningModel?.displayName || reasoningModel?.name || active.model || "modelo"}</span>{activeReasoningEffort && <span>esfuerzo · {activeReasoningEffort}</span>}{active.repo && <span>repo · {active.repo}</span>}{active.queuedSupervisor.length > 0 && <span className={s.queueChip}>{active.queuedSupervisor.length} en cola</span>}{active.pullRequests.length > 0 && <button type="button" className={s.prQuickButton} aria-label={`Abrir lista de ${active.pullRequests.length} Pull Requests`} aria-expanded={prQuickOpen} onClick={() => setPrQuickOpen((open) => !open)}>PRs · {active.pullRequests.length}</button>}</div>
