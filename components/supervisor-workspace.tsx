...
export default function SupervisorWorkspace() {
  ...
  const canSendSupervisor = Boolean(active?.draft.trim() && providerReady && (active.provider === "apple" ? active.mode === "chat" : active.mode === "chat" || active.repo) && (active.mode !== "pr" || githubConnected));
  ...
  function handleSupervisorEvent(chatId: string, runId: string, provider: Provider, model: string, event: StreamEvent) {
    ...
    if (event.type === "run.started") { system(chatId, `Supervisor inició · ${providerLabel(provider)} · ${model || "auto"}`, "good"); return; }
  ...
  async function startSupervisorRun(chatId: string, explicitPrompt?: string, alreadyAppended = false) {
    ...
    try {
      if (thread.provider === "apple") {
        ...
        updateChat(chatId, (chat) => ({ ...chat, running: false, runStatus: "completed", status: "Apple Intelligence listo", error: "", updatedAt: Date.now() }));
        return;
      }
