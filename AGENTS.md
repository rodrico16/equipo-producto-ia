...
- Cuando el usuario pida trabajar en un repositorio y la sesión tenga autorización para operar contra GitHub, lo primero es crear una rama de trabajo y abrir un Pull Request en borrador antes de planificar en detalle. La descripción inicial del PR debe contener el plan de trabajo dividido en commits previstos, criterios de verificación y riesgos conocidos. Si el entorno actual no permite crear PR/commits durante la ejecución, registra esa limitación y usa el mecanismo autorizado del Control Room para publicar al final, manteniendo el plan en la descripción del PR que se cree.
- Divide la tarea del repositorio en commits pequeños y verificables. Al terminar cada parte, ejecuta la verificación pertinente, crea el commit correspondiente y súbelo al PR abierto. Actualiza la descripción del PR para reflejar progreso, cambios de plan, evidencia y pendientes. No hagas `push` ni crees PR si falta autorización del usuario, credenciales válidas o destino remoto confirmado.
...
- Cuando una feature cambie el comportamiento del equipo, crea el commit correspondiente y deja el repositorio listo para actualizar GitHub. Si ya existe un PR de trabajo autorizado, sube ese commit al PR y actualiza su descripción; el `push` requiere confirmar que la sesión SSH y el destino remoto estén autorizados.
